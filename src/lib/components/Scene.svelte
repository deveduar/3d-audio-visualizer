<script lang="ts">
    ...
    useTask(() => {
        ...
        if (p.displayMode === "tunnel") {
            ...
            lightB +=
                (0.75 + impact * 0.8 + p.tunnelPulse * 0.25 - lightB) * 0.14;
            return;
        }

        const reactiveEnabled =
            p.cameraMode === "reactive" || p.cameraMode === "orbit-reactive";
        const audioMultiplier = p.audioReactAmount;
        const dampingFactor =
            (0.04 + p.cameraDamping * 0.3) * (audioMultiplier > 0 ? 1 : 0.05);
        ...
        if (reactiveEnabled && p.cameraReactiveAmount > 0.01) {
            eventIntensity =
                impact *
                p.cameraReactiveAmount *
                (0.8 + p.impactFrame * 1.6) *
                audioMultiplier;
            ...
            targetZ =
                -eventIntensity * (8 + p.impactFlash * 5) -
                low * 5 * p.cameraReactiveAmount * audioMultiplier;
        }
        ...
        lightA +=
            (1 +
                low * 0.45 * audioMultiplier +
                eventIntensity * (1.3 + p.impactFlash) -
                lightA) *
            0.12;
        lightB +=
            (0.5 + eventIntensity * (0.7 + p.impactFrame * 0.55) - lightB) *
            0.12;
    });
    ...
    <T.Group position={[cameraX, cameraY, cameraZ]} rotation.z={rigRotationZ}>
        ...
        >
            <OrbitControls
                ...
                minDistance={10}
                maxDistance={800}
                enableRotate={$params.displayMode !== "tunnel" &&
                    ($params.cameraMode === "orbit" ||
                        $params.cameraMode === "orbit-reactive")}
                enableZoom={false}
                enablePan={$params.displayMode !== "tunnel" &&
                    $params.cameraEnablePan}
                enableDamping
                dampingFactor={0.08}
                rotateSpeed={0.8}
            />
